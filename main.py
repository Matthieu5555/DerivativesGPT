...
async def main(message: cl.Message) -> None:
    ...
    try:
        ...
        await cl.Message(
            content=f"Oh no! it seems that Matthieu & Alexandro didn't fix their code before shelving it, please contact them to let them know!\n\n"
    ...
    try:
        ...
        # Check if it's an OpenRouter credit/auth error (Joke 1)
        error_str = str(e).lower()
        if any(keyword in error_str for keyword in ['openrouter', 'auth', 'credit', 'insufficient', 'rate limit', '402', '429']):
            await cl.Message(
                content="Send a linkedin message to Matthieu Separt to give his credit card back to Open Router to be able to get a response"
            ).send()
        else:
            # Generic error - Joke 3
            await cl.Message(
                content=f"Oh no! it seems that Matthieu & Alexandro didn't fix their code before shelving it, please contact them to let them know!\n\n"
