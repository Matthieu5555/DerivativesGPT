...
# Controls decimal places in all price displays ($12.34 vs $12.3456).
...
# Threshold for classifying option as "at-the-money".
...
# Number of trading days to show in price history charts.
...
# Days of historical data to fetch when ticker not in database.
...
# Timeout for yfinance API calls to prevent hanging.
...
# Default volatility when SQL database AND yfinance both fail.
...
# Default risk-free rate when config AND estimation both fail.
...
# Provides realistic volatility estimates for testing.
...
# Maximum number of times to retry validation before giving up.
