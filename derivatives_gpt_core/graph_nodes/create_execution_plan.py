...
async def create_execution_plan(state: BaseAgentState) -> Dict[str, Any]:
    """
    ...
    if state.legs and len(state.legs) > 0:
        ...
        context_parts.append(f"Single option: {state.option_type or 'call'}")
    ...
    if state.legs:
        ...
        legs_description = f"\nSingle option to price: {state.option_type or 'call'}"
