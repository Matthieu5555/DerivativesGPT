...
async def verify_understanding(state: EducationalState) -> dict:
    ...
    questions_message = _format_verification_message(questions)

    # BUG FIX: Combine explanation with verification questions instead of replacing
    # The explanation_text was already added to messages by generate_explanation node
    # We need to combine them into a single message, not overwrite
