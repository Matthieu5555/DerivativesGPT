...
import logging
from derivatives_gpt_core.agents.educational.state import EducationalState as BaseAgentState  # Legacy
...
def route_after_comprehension(state: BaseAgentState) -> NodeName:
    """
    Route after user comprehension assessment.

    Routes:
    - High comprehension (score ≥ 4) → reflect_on_learning (done)
    - Medium comprehension (score 3) → reflect_on_learning (acceptable)
    - Low comprehension (score ≤ 2) → clarify_confusion OR rewrite_explanation

    This activates the human-in-the-loop feedback cycle.

    Args:
        state: State with user_understanding_score
    ...
    understanding_score = wrapped.get_field("user_understanding_score", 3)
    confusion_signals = wrapped.get_field("user_confusion_signals", [])

    if understanding_score >= 4:
        logger.info(f"Strong comprehension (score: {understanding_score}), proceeding to reflection")
        return "reflect_on_learning"
    elif understanding_score == 3:
        logger.info(f"Acceptable comprehension (score: {understanding_score}), proceeding to reflection")
        return "reflect_on_learning"
    else:
        # Low comprehension - need to re-explain
        if confusion_signals:
            logger.warning(f"Low comprehension (score: {understanding_score}), confusion detected: {confusion_signals[:2]}")
        else:
            logger.warning(f"Low comprehension (score: {understanding_score}), will rewrite explanation")

        # For now, rewrite the explanation to address gaps
        # Could add a separate "clarify_confusion" node in future
        return "rewrite_explanation"
